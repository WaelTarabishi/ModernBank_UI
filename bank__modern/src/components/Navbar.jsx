import { useState } from "react";
import { close, logo, menu } from "../assets";
import { navLinks } from "../constants/index";
const Navbar = () => {
  const [toggel, setToggel] = useState(false);
  return (
    <nav className="w-full justify-between p-6 navbar flex items-center">
      <img src={logo} alt="logo" className="w-[124px] h-[32px]" />
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`font-poppins font-normal cursor-pointer text-[16px] text-white ${index === navLinks.length - 1 ? "mr-0" : "mr-10"
              }`}
          >
            <a href={`#${nav.id}`}>{nav.title}</a>
          </li>
        ))}
      </ul>
      <div className="sm:hidden flex flex-1 justify-end items-center">
        <img
          src={toggel ? close : menu}
          alt="menu"
          className="w-[24px] h-[24px] object-contain "
          onClick={() => setToggel((prev) => !prev)}
        />
        <div
          className={`${toggel ? "flex" : "hidden"
            } bg-black-gradient p-6 absolute top-20 right-0 mx-4 my-2 min-w-[124px] rounded-xl sidebar `}
        >
          <ul className="list-none flex-col  justify-center items-center flex-1">
            {navLinks.map((nav, index) => (
              <li
                key={nav.id}
                className={`font-poppins font-normal cursor-pointer text-[16px] text-white ${index === navLinks.length - 1 ? "mb-0" : "mb-4"
                  }`}
              >
                <a href={`#${nav.id}`}>{nav.title}</a>
              </li>
            ))}
          </ul>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
